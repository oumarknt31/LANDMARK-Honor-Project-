#include "quizmathsdialog.h"
#include "ui_quizmathsdialog.h"
...
#include <QDebug>
#include <QPixmap>
#include "quizmathsdialog.ui"



quizmathsDialog::quizmathsDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::quizmathsDialog)
{
    ui->setupUi(this);

    /*QPixmap mathsQPic(":/img/ComputerSc quiz.jpeg");
    ui->CscQuizPic->setPixmap(calcIQPic);

    QPixmap calcIQPic(":/img/Csc111Quiz.jpeg");
    ui->CSC111_Pic_calc->setPixmap(csc111QPic);

    QPixmap calcIIPic(":/img/Csc211Quiz.jpeg");
    ui->CSC211_Pic_calc->setPixmap(calcIIPic);

    QPixmap mbackgrdQPic(":/img/Quiz Background.jpeg");
    ui->QuizCsc111Pic_calc->setPixmap(mbackgrdQPic);

    QPixmap msummaryQPic(":/img/Quiz Background.jpeg");
    ui->summaryPic_calc->setPixmap(msummaryQPic);


    QPixmap mbackgrdQPicII(":/img/Quiz Background.jpeg");
    ui->QuizCsc211Pic->setPixmap(mbackgrdQPicII);

    QPixmap msummaryQPicII(":/img/Quiz Background.jpeg");
    ui->summaryPic_II->setPixmap(msummaryQPicII);*/
...
quizmathsDialog::~quizmathsDialog()
{
    delete ui;
}
/*
int QuizCscDialog::initializeQuestions(int index) {
    QVector<QuestionQuiz> question;
...
void QuizCscDialog::on_Confirm_btn_clicked()
{
    if (position >= 0 && position < 10) {
        int correctOption = initializeQuestions(position);
        if (correctOption != -1) { // Check for a valid return value
            ...
            position++;
            int correctOption2 = initializeQuestions(position);
            if (position == 10){
...
int QuizCscDialog::initializeQuestionsII(int index) {
...
void QuizCscDialog::on_Confirm_btn_II_clicked()
{
    if (positionII >= 0 && positionII < 10) {
        int correctOptionII = initializeQuestionsII(positionII);
        if (correctOptionII != -1) { // Check for a valid return value
            ...
            positionII++;
            int correctOptionII2 = initializeQuestionsII(positionII);
                if (positionII == 10){
    ...
    if (positionII==10){
        ...
        else if (pointsII==8 || pointsII == 9){
            ui->Congrats_lbl_II->setText("EXCELLENT WORK!");
        }
        else if(pointsII==7 || pointsII ==6){
